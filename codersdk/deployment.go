...
Write out the current server config as YAML to stdout.`,
		...
		},
		{
			...
			Description: "Number of most recent attachments a chat keeps, counting uploads and files the agent attaches from the workspace. Older attachments are removed when the cap is reached, and a single message cannot include more files than the cap.",
		...
		},
		{
			...
			Description: "Maximum number of virtual desktop recordings the chat daemon stores concurrently. Each upload can buffer about 110 MB in memory, so this bounds the daemon's peak memory use for recordings.",
